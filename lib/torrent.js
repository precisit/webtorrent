...
Torrent.prototype._onSwarmListening = function () {
  ...
  self.discovery.on('jsflow_peer', self.addJsFlowPeer.bind(self))
...
/**
 * Add a JsFlow peer to the swarm
 * @param {string|SimplePeer} peer
 * @return {boolean} true if peer was added, false if peer was blocked
 */
Torrent.prototype.addJsFlowPeer = function (peer) {
  var self = this
  console.log('Inside jsflow peer!');
  
  // TODO: extract IP address from peer object and check blocklist
  if (typeof peer === 'string'
      && self.client.blocked
      && self.client.blocked.contains(peer)) {
    self.numBlockedPeers += 1
    self.emit('blockedPeer', peer)
    return false
  } else {
    self.emit('jsflow_peer', peer)
    console.log('WILL ADD PEER to JSFLOW PEER !');
    self.swarm.addJsFlowPeer(peer)
    return true
  }
}
